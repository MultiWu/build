...
class source:
    ...
    def search(self, title, localtitle, year=''):
        try:
            ...
            for title in titles:
                ...
                self.session.post('https://xt7.pl/mojekonto/szukaj', data=data)
                ...
                headers = {
                    ...
                    'Referer': 'https://xt7.pl/mojekonto/szukaj/1',
                ...
                self.session.post('https://xt7.pl/mojekonto/szukaj/1', headers=headers,
                                  data=data)
                r = self.session.post('https://xt7.pl/mojekonto/szukaj/1', headers=headers,
                                      data=data).content
    ...
    def sources(self, rows, hostDict, hostprDict):
        ...
        try:
            if type(rows) == tuple:
                ...
                sources.append(
                    {'source': 'xt7', 'quality': quality, 'language': info[0], 'url': rows[0],
                     'info': 'Moje Pliki',
                     'direct': True,
                     'debridonly': False})
                return sources
            for row in rows:
                try:
                    ...
                    sources.append(
                        {'source': 'xt7', 'quality': quality, 'language': info[0], 'url': link,
                         'info': size + ' ' + info2,
                         'direct': True,
                         'debridonly': False})
                except:
    ...
    def resolve(self, url):
        ...
        content = self.session.post('https://xt7.pl/mojekonto/sciagaj', data=data, headers=headers).content

...
class source:
    ...
    def search(self, title, localtitle, year):
        try:
            cookies = client.request("http://filmdom.fun/", output='cookie')
            cache.cache_insert('filmdom_cookie', cookies)
            titles = []
            ...
            for title in titles:
                title = title.replace(" ", "+")
                result = client.request(self.search_link % title, cookie=cookies)
    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        try:
            ...
            cookies = cache.cache_get('filmdom_cookie')['value']
    ...
    def resolve(self, url):
        cookies = cache.cache_get('filmdom_cookie')['value']
        result = client.request(url, cookie=cookies)
        link = client.parseDOM(result, 'iframe', ret='src')[0]
        return link

...
class source:
    def __init__(self):
        self.priority = 1
        self.language = ['pl']
        self.domains = ['otube.pl']

        self.base_link = 'https://otube.pl/'
            ...
            for title in titles:
                ...
                for item in result:
                    ...
                    if self.contains_all_words(name, words):
                        ...
                        #info = client.parseDOM(result2, 'div', attrs={'class': 'info'})[0]
                        result2 = client.parseDOM(result2, 'ul', attrs={'id': 'episode-list'})
                        result2 = client.parseDOM(result2, 'ul')
                        for item2 in result2:
                            if season_episode.lower() in item2:
                                link = client.parseDOM(result2, 'a', ret="href")[0]
                                return link

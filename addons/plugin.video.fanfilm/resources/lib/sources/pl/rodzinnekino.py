...
class source:
    def __init__(self):
        self.priority = 1
        self.language = ['pl']
        self.domains = ['rodzinnekino.com']

        self.base_link = 'https://rodzinnekino.com/'
    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        try:
            ...
            result2 = client.parseDOM(result, 'li', attrs={'class': 'dooplay_player_option'})
            if not result2:
                result2 = client.parseDOM(result, 'li', attrs={'class': 'dooplay_player_option jump'})
            for item in zip(result1, result2, result3):
                try:
                    ...
                    sources.append(
                        {'source': host, 'quality': 'SD', 'language': jezyk, 'url': str(item[0] + ";" + item[2]),
    ...
    def resolve(self, url):
        ...
        response = s.post('https://rodzinnekino.com/wp-admin/admin-ajax.php', data=data).text
        url = client.parseDOM(response, 'iframe', ret='src')[0]
        url = s.get(url, allow_redirects=False).headers.get('Location')

...
def absoluteNumber(tvdb, episode, season):
    ...
    ADDON = xbmcaddon.Addon('plugin.video.fanfilm')
    apikey = str(ADDON.getSetting('TVDb_ApiKeyF'))
    userkey = str(ADDON.getSetting('TVDb_UserKeyF'))
    username = str(ADDON.getSetting('TVDb_UsernameF'))

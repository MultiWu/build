...
class CloudflareScraper(Session):
    ...
    def solve_cf_challenge(self, resp, **original_kwargs):
        ...
        try:
            params = cloudflare_kwargs["data"] = OrderedDict(re.findall(r'name="(r|s|jschl_vc|pass)"(?: [^<>]*)? value="(.+?)"', body))
            ...
            if len(sections) < 3:
                if '/' in sections[1]:
                    ...
                    if char_code_at_sep in subsecs[1]:
                        ...
                        operand_2 = ord(domain[parseJSString(subsecs[1][subsecs[1].find(char_code_at_sep) + len(char_code_at_sep):-2])])

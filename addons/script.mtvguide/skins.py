...
import os, re, sys, shutil, zipfile, StringIO
import xbmcgui, xbmc
from strings import *
...
class Skin:
    ...
    if KODI_VERSION >= 17:
        NEW_SKINS_BASE_URL      = M_TVGUIDE_SUPPORT + 'skins_release/'
    else:
        NEW_SKINS_BASE_URL      = M_TVGUIDE_SUPPORT + 'skins_legacy/'
    ...
    def getDefaultSkinBaseDir():
        if os.path.isdir(os.path.join(Skin.ADDON_CUSTOM_SKINS, 'Default')):
    ...
    def fixSkinIfNeeded():
        ...
        if Skin.getSkinPath() is None:
            ...
            if success == False:
                ADDON.setSetting(id="Skin", value=str('Default'))
    ...
    def createCustomDirIfNeeded():
        if os.path.isdir(Skin.ADDON_CUSTOM_SKINS) == False:
            ...
            try:
                shutil.copytree(os.path.join(Skin.ADDON_EMBEDDED_SKINS, 'Default'), os.path.join(Skin.ADDON_CUSTOM_SKINS, 'Default'))
            except Exception, ex:
...
class SkinPicker(xbmcgui.WindowXMLDialog):
    def __new__(cls):
        return super(SkinPicker, cls).__new__(cls, 'script-tvguide-skins.xml', Skin.getDefaultSkinBaseDir(), 'Default', '720p')
    ...
    def setFanartImage(self, skin):
        if skin:
            self.skin_image_control.setImage(skin.fanart.encode('utf-8'))
            self.skin_name_control.setImage(skin.icon.encode('utf-8'))
    ...
    def setCurrentSkin(self):
        try:
            if self.currentlySelectedSkin is not None:
                ...
                deb('setCurrentSkin selected skinn is none!')
        except:
    ...
    def onClick(self, controlId):
        ...
        if clicked_control == self.go_down_control:
            ...
            self.showNextSkin()
        elif clicked_control == self.go_up_control:
            deb('SkinPicker goup control')
            self.showPreviousSkin()
        elif clicked_control == self.select_control:
